...
class MultiLanguageAIAgencyChatbot:
    def __init__(self, index_name: str = "ai-agency-chatbot"):
        # Load environment variables
        load_dotenv()
        
        # Supported languages mapping - focused on your specific requirements
    ...
    },
    {
        ...
        self.vector_search = QdrantVectorSearch(index_name=index_name)
        self.vector_search.connect()

        # --- Build Index with Agency Data ---
        self.vector_search.build(self.agency_data)
        print("✅ Initialized Qdrant vector search and upserted agency data!")

        # --- Initialize Chat Models and History ---
        self.llm = ChatOpenAI(model="gpt-4o", temperature=0.3, max_tokens=1000)
        # Use GPT-4 for translations as well for consistency and quality
        self.translation_llm = ChatOpenAI(model="gpt-4", temperature=0)
        self.message_history = ChatMessageHistory()

        # --- Define Language-Aware Classification Prompt ---
        self.classify_query_prompt = PromptTemplate(
            input_variables=["user_query", "detected_language"],
            template="""You are an AI assistant that helps users with AI agency recommendations in multiple languages.
...
"""),
    ...
    def classify_query_with_prompt(self, user_query: str, detected_language: str) -> str:
        """
        Classifies the user query into categories, considering the detected language.
        """
        # Translate query to English for classification if needed
        english_query = self.translate_query_to_english(user_query, detected_language)
        
        # Format the prompt with the user query and detected language
        prompt = self.classify_query_prompt.format(
            user_query=english_query, 
            detected_language=self.supported_languages.get(detected_language, 'English')
        )
    ...
    def send_agency_recommendation_to_backend(self, agency_ids: list, user_language: str):
        ...
        # Uncomment and modify when you have your actual backend URL
...
#         """
#         Generates a chat title based on the first 3 user messages.
#         The title is generated only once after the 3rd user message and never changes after that.
        
#         Returns:
#             str: A concise title representing the main topic of the conversation
#         """
#         try:
#             # If title is already generated and cached, return it (never regenerate)
#             if hasattr(self, '_final_title') and self._final_title:
...
#             user_messages = []
#             for msg in messages:
#                 # Check if it's a user message (different message types based on LangChain version)
#                 if hasattr(msg, 'type') and msg.type == 'human':
#                     user_messages.append(msg.content)
#                 elif hasattr(msg, '__class__') and 'Human' in msg.__class__.__name__:
#                     user_messages.append(msg.content)
            
#             # Only generate title if we have exactly 3 or more user messages
#             if len(user_messages) < 3:
#                 return "New Conversation"  # Temporary title until 3rd message
            
#             # Use first 3 user messages to generate the final title
#             first_three_messages = user_messages[:3]
#             combined_context = " ".join(first_three_messages)
            
#             # Detect language from the conversation
#             detected_language = self.detect_language(combined_context)
            
#             # Generate title using LLM
#             title_prompt = f"""Based on these first 3 messages from a user chatting with an AI agency recommendation bot, generate a short, descriptive title (3-5 words maximum) that captures the main topic or need.

# Rules:
# 1. Maximum 40 characters
# 2. Be specific about the main topic/industry/use case
# 3. Respond in {self.supported_languages.get(detected_language, 'English')}
# 4. Focus on what the user wants to build/solve
# 5. Avoid generic words like "chat", "conversation", "help"
# 6. If about AI/tech projects, mention the specific domain (e.g., "E-commerce Chatbot", "Healthcare Analytics")

# User's first 3 messages:
# {combined_context[:400]}

# Generate only the title (no quotes, no extra text):"""

#             # Generate title
#             result = self.translation_llm.invoke(title_prompt)
#             title = result.content.strip().replace('"', '').replace("'", "")
            
            
#             # Cache the final title permanently
#             self._final_title = title
            
#             # Optional: Send to backend
#             self._send_final_title_to_backend(title)
            
#             return title
            
#         except Exception as e:
#             print(f"Error generating title: {e}")
#             # Cache a default title so it doesn't keep trying
#             self._final_title = "AI Agency Chat"
#             return self._final_title


#     def _send_final_title_to_backend(self, title: str):
#         """
#         Sends the final chat title to backend (called only once after 3rd message).
#         """
#         try:
#             data = {
...
#             print(f"🏷️ Final chat title set: {title}")
            
#             # Uncomment when you have your backend endpoint
#             # backend_url = "https://your-backend-url.com/set-chat-title"
#             # response = requests.post(backend_url, json=data)
            
#         except Exception as e:
#             print(f"Failed to send title to backend: {e}")

#     def has_title_been_generated(self) -> bool:
#         """
#         Check if the final title has already been generated.
        
#         Returns:
#             bool: True if title is already set, False otherwise
#         """
#         return hasattr(self, '_final_title') and self._final_title is not None

#     def get_current_title_status(self) -> dict:
#         """
#         Get the current status of title generation.
        
#         Returns:
#             dict: Status information about the title
#         """
#         user_message_count = len([msg for msg in self.message_history.messages 
#                                  if hasattr(msg, 'type') and msg.type == 'human'])
        
#         return {
#             "user_messages": user_message_count,
#             "title_generated": self.has_title_been_generated(),
#             "current_title": getattr(self, '_final_title', "New Conversation"),
#             "needs_more_messages": user_message_count < 3 and not self.has_title_been_generated()
#         }

#     # ============================================================================
#     # END OF TITLE GENERATION FUNCTIONS
#     # ============================================================================
    def get_title(self) -> str:
        """
        Generates a chat title based on the user messages with predefined titles for common scenarios.
        ...
        if not combined_context.strip() or user_message_count == 0:
            return "New Chat"
        ...
        casual_patterns = {
            ...
            'es': {
                ...
                'status': ['cómo estás', 'qué tal', 'bien', 'muy bien'],
                'introductions': ['mi nombre es', 'soy', 'mucho gusto', 'quién eres'],
                'titles': {
                    'greetings': 'Charla Casual',
                    'status': 'Conversación General',
                    'introductions': 'Chat de Presentación'
                }
            },
            'fr': {
                'greetings': ['bonjour', 'bonsoir', 'salut'],
                'status': ['comment allez-vous', 'ça va', 'bien', 'très bien'],
                'introductions': ['je m\'appelle', 'je suis', 'enchanté', 'qui êtes-vous'],
                'titles': {
                    'greetings': 'Discussion Décontractée',
                    'status': 'Conversation Générale',
                    'introductions': 'Chat de Présentation'
                }
            },
            'de': {
                'greetings': ['hallo', 'guten morgen', 'guten tag', 'guten abend'],
                'status': ['wie geht es', 'wie gehts', 'gut', 'sehr gut'],
                'introductions': ['ich heiße', 'ich bin', 'freut mich', 'wer sind sie'],
                'titles': {
                    'greetings': 'Lockeres Chat',
                    'status': 'Allgemeine Unterhaltung',
                    'introductions': 'Vorstellungs-Chat'
                }
            },
            'nl': {
                'greetings': ['hallo', 'goedemorgen', 'goedemiddag', 'goedenavond'],
                'status': ['hoe gaat het', 'goed', 'prima', 'uitstekend'],
                'introductions': ['ik heet', 'ik ben', 'aangenaam', 'wie ben je'],
                'titles': {
                    'greetings': 'Casual Gesprek',
                    'status': 'Algemene Conversatie',
                    'introductions': 'Introductie Chat'
                }
            },
            'bn': {
                'greetings': ['হ্যালো', 'নমস্কার', 'সুপ্রভাত', 'শুভ সন্ধ্যা'],
                'status': ['কেমন আছেন', 'ভালো', 'খুব ভাল'],
                'introductions': ['আমার নাম', 'আমি', 'খুশি হলাম', 'আপনি কে'],
                'titles': {
                    'greetings': 'নৈমিত্তিক চ্যাট',
                    'status': 'সাধারণ কথোপকথন',
                    'introductions': 'পরিচিতি চ্যাট'
                }
            }
        }
        ...
        # If only casual conversation (no project/agency mentions)
        ...
        title_prompt = f"""Based on these user messages from an AI agency recommendation chat, generate a concise title (2-4 words, max 35 characters) in {language_name}.

    TITLE RULES:
    1. Be specific about the main topic/industry/domain
    2. Use keywords that describe what user wants to build/solve
    3. Avoid generic words like "chat", "conversation", "help", "question"
    4. Focus on the business need or AI solution type
    5. Examples of good titles:
    - "Finance related AI Analytics" (not "Finance Help")
    - "E-commerce related Chatbot" (not "Chatbot Questions") 
    - "Healthcare related Data Mining" (not "Data Help")
    - "Retail related Computer Vision" (not "AI Conversation")
    ...
    Generate ONLY the title (no quotes, explanations, or extra text):"""
    ...
    def get_response(self, user_input: str):
        ...
        try:
            ...
            print("\n⚠️ Response generation was interrupted. Please try again.")
            return "Lo siento, la generación de respuesta fue interrumpida. Por favor, inténtalo de nuevo." if detected_language == 'es' else "Sorry, response generation was interrupted. Please try again."
...
if __name__ == "__main__":
    chatbot = MultiLanguageAIAgencyChatbot()
    print("\n🤖 Multi-Language AI Agency Chatbot is ready! Type 'exit' to quit.")
    print("Supported languages: English, Bangla, French, Dutch, German, Spanish")
    print("💡 Tip: If response generation takes too long, press Ctrl+C to interrupt and try again.\n")

    while True:
        try:
            user_input = input("User: ")
            if user_input.lower() in ["exit", "quit", "বের হন", "quitter", "afsluiten", "beenden", "salir"]:
                print("Goodbye! বিদায়! Au revoir! Tot ziens! Auf Wiedersehen! ¡Adiós!")
                break
